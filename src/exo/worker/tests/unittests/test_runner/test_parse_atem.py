...
def _collect_with_finish_reason(text: str, finish_reason: FinishReason):
    responses = (
        response
        for response in [
            GenerationResponse(
...
def test_plain_text_passes_through_and_state_is_per_request():
    ...
    assert (
        "".join(item.text for item in first if isinstance(item, GenerationResponse))
        == "plain answer"
    )
    assert isinstance(first[-1], GenerationResponse)
    assert isinstance(second[0], GenerationResponse)
...
def test_plain_text_with_empty_terminal_chunk_stops_normally():
    ...
    assert (
        "".join(item.text for item in results if isinstance(item, GenerationResponse))
        == "plain answer"
    )
...
def test_reasoning_and_visible_content_survive_every_character_split():
    output = (
        "to=self<|message|>private plan<|eom|>to=user<|message|>public answer<|eot|>"
    )
    ...
    for split in range(len(output) + 1):
        ...
        assert (
            "".join(item.text for item in generated if item.is_thinking)
            == "private plan"
        )
        assert (
            "".join(item.text for item in generated if not item.is_thinking)
            == "public answer"
        )
        assert generated[-1].finish_reason == "stop"
        assert all(
            "<|" not in item.text and "to=" not in item.text for item in generated
        )
...
def test_actual_muse_assistant_framing_is_parsed():
    ...
    assert (
        "".join(item.text for item in generated if item.is_thinking) == "private plan"
    )
    assert "".join(item.text for item in generated if not item.is_thinking) == "126"
    assert generated[-1].finish_reason == "stop"


def test_muse_eot_stop_token_is_restored_for_parsing():
    responses = (
        response
        for response in [
            GenerationResponse(
                text=(
                    "to=self<|message|>private plan<|eom|>"
                    "<|start|>assistant to=user<|message|>126"
                ),
                token=0,
                finish_reason=None,
                usage=None,
    ...
    results = list(parse_atem_output(responses, TOOLS))
    generated = [item for item in results if isinstance(item, GenerationResponse)]
...
def test_apply_all_parsers_routes_muse_tool_calls_to_structured_chunk():
    ...
    chunks = [
        ...
        for chunk in apply_all_parsers(
            ...
            model_type=cast(type[Model], MuseGlimmerModel),
