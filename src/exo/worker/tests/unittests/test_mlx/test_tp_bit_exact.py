...
MODEL_CONFIGS = {
    "glm5_next": {
        "module": "exo.worker.engines.mlx.glm5_next",
        "args": {
            "text_config": {
                "vocab_size": 512,
                "hidden_size": 128,
...
def _build(name, seed=0):
    ...
    cfg = MODEL_CONFIGS["glm5_next" if name == "glm5_next_quantized" else name]
    ...
    mx.random.seed(seed)
    args = (
        model_args_cls.from_dict(cfg["args"])
        if name.startswith("glm5_next")
        else model_args_cls(**cfg["args"])
    )
    m = model_cls(args)
    ...
    if name == "glm5_next_quantized":
        nn.quantize(
            m,
            group_size=64,
            bits=4,
            class_predicate=lambda path, module: (
                {"bits": 5 if path.endswith("down_proj") else 4, "group_size": 64}
        ...
        if muse_attention_weights is not None:
            ...
            for layer, (query_weight, gate_weight) in zip(
                ...
                for projection, source in (
                    ...
                    expected = source.reshape(n_kv_heads, heads_per_kv, head_dim, -1)[
                        :, start:end
...
def test_glm5_next_tp_numerical_parity(quantized):
    name = "glm5_next_quantized" if quantized else "glm5_next"
    _run_compare(name, 4, 32184, atol=0.05, rtol=0.01)
