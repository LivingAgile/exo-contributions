...
def test_muse_glimmer_processor_and_checkpoint_key_sanitization():
    ...
    processed = processor(images=Image.fromarray(np.zeros((28, 28, 3), dtype=np.uint8)))
...
def test_exo_vision_encoder_loads_and_encodes_official_nested_muse_weights(
    ...
    official_weights = {
        **{
            ...
            for name, value in cast(
                ...
                tree_flatten(expected.vision_tower.parameters()),
            )
        },
        **{
            ...
            for name, value in cast(
                list[tuple[str, mx.array]],
                tree_flatten(expected.vision_adapter.parameters()),
            )
        },
        **{
            ...
            for name, value in cast(
                list[tuple[str, mx.array]],
                tree_flatten(expected.vision_projection.parameters()),
            )
        },
        **{
            ...
            for name, value in cast(
                list[tuple[str, mx.array]],
                tree_flatten(expected.perception_emb_norm.parameters()),
            )
        },
    }
    ...
    def path_resolver(_model_id: ModelId) -> Path:
        return tmp_path

    def processor_loader(_path: Path) -> MuseGlimmerImageProcessor:
        return processor

    monkeypatch.setattr(vision_module, "build_model_path", path_resolver)
    monkeypatch.setattr(vision_module, "load_image_processor", processor_loader)
    ...
    assert vars(encoder)["_projector"] is not None
    assert features.shape[-1] == 32
    assert token_counts == [1]

    official_weights.pop("model.vision_projection.weight")
    mx.save_safetensors(str(tmp_path / "model.safetensors"), official_weights)
    incomplete_encoder = VisionEncoder(
        vision_config, ModelId(vision_config.weights_repo)
    )
    with pytest.raises(
        ValueError,
