...
def _parse_atem_parameter(text: str, schema: dict[str, Any]) -> Any:
    schema_type = schema.get("type")
    if isinstance(schema_type, list):
    ...
    if schema_type == "string":
        ...
        try:
            ...
            return _INVALID_ATEM_VALUE
        converted = _validate_atem_json_value(converted, schema)
        if converted is _INVALID_ATEM_VALUE:
            return _INVALID_ATEM_VALUE
    else:
        return _INVALID_ATEM_VALUE
...
def _validate_atem_json_value(value: Any, schema: dict[str, Any]) -> Any:
    schema_type = schema.get("type")
    ...
    if schema_type == "array":
        ...
        if not isinstance(item_schema, dict):
            return value
        converted_items = [_validate_atem_json_value(item, item_schema) for item in value]
