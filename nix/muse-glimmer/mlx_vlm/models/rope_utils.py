...
def _eager_rope_angles(x, offset, frequencies, scale, traditional):
    positions = mx.arange(x.shape[-2], dtype=mx.float32)[None, :] + offset[..., None]
...
class EagerRoPE(nn.Module):
    def __init__(
        ...
        mx.eval(self._frequencies, self._scale)
...
def _rotary_apply_kernel(
    ...
    if sectioned:
        cos_expr = (
            f"cos[((axis * q_bsz + b) * q_len + t) * {rotary_dim} + {cos_freq_idx}]"
        )
        sin_expr = (
            f"sin[((axis * q_bsz + b) * q_len + t) * {rotary_dim} + {cos_freq_idx}]"
        )
        pair_cos_expr = (
        ...
        pair_cos_expr = (
            f"cos[((0 * q_bsz + b) * q_len + t) * {rotary_dim} + {pair_cos_freq_idx}]"
        )
        pair_sin_expr = (
            f"sin[((0 * q_bsz + b) * q_len + t) * {rotary_dim} + {pair_cos_freq_idx}]"
        )
        selector_source = ""
